...
void stack_printer(int *stack, int bp, int sp, int *inc, FILE *outFile)
{
  int sum = 0;
  int opps = 0;
  int f = 0;
      ...
      switch (code[ir].op)
      {
        // LIT
         case 1:
              ...
              stack_printer(stack, bp, sp, inc, outFile);
              break;
         // OPR
         case 2:
               switch (code[ir].m)
         ...
         case 11:
              fprintf(outFile, "%d\t%d\t%d\t", pc, bp, sp);
              for(int i = MAX_DATA_STACK_HEIGHT - 1; i >= MAX_DATA_STACK_HEIGHT - inc[0]; i--)
              {
